...
class Attn_Condenser_Block(nn.Module):
    def __init__(self, name, in_channels, mid_channels_0, out_channels_0,
        ...
        self.pool_0 = nn.MaxPool2d(kernel_size=2, stride=2, return_indices=True)
        ...
        self.unpool_0 = nn.MaxUnpool2d(kernel_size=2, stride=2)
        ...
        self.group_conv_1 = nn.Conv2d(out_channels_0, mid_channels_1, kernel_size=3,
                                      padding="same", groups=out_channels_0)
    ...
    def forward(self, x):
        V_0 = x
        Q_0, indices_0 = self.pool_0(V_0)
        K_0 = F.leaky_relu(self.group_conv_0(Q_0))
        K_0 = F.leaky_relu(self.pointwise_conv_0(K_0))
        K_0_expend = F.leaky_relu(self.expand_conv_0(K_0))
        A_0 = self.unpool_0(K_0_expend, indices_0, output_size=V_0.shape)
        #A_0_expend = F.leaky_relu(self.expand_conv_0(A_0))
        V_1 = V_0 * self.S_0 + A_0
        if self.batch_norm:
            V_1 = self.batch_nrom_0(V_1)
        #Q_1, indices_1 = self.pool_1(A_0)
        K_1 = F.leaky_relu(self.group_conv_1(K_0))
        K_1 = F.leaky_relu(self.pointwise_conv_1(K_1))
        A_1 = self.unpool_1(K_1, indices_0, output_size=V_1.shape)
