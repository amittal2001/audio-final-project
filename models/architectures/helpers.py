...
class BitBatchNorm2d(nn.BatchNorm2d, BitQuant):
    ...
    def forward(self, x):
        ...
        y = F.batch_norm(x_quant, self.running_mean, self.running_var, w_quant, b_quant, self.training, self.momentum,
                         self.eps)
...
class BitConv2d(nn.Conv2d, BitQuant):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, groups=1, QuantType='4bitsym',
                 WScale='PerTensor', NormType='RMS', quantscale=0.25):
        nn.Conv2d.__init__(self, in_channels, out_channels, kernel_size=kernel_size, stride=(stride,),
...
class QAttentionCondenser(nn.Module):
    def __init__(self, in_channels, mid_channels, out_channels, QuantType='4bitsym', WScale='PerTensor', NormType='RMS',
                 quantscale=0.25):
        super(QAttentionCondenser, self).__init__()
        self.condense = nn.MaxPool2d(kernel_size=2, stride=2)
        self.group_conv = BitConv2d(in_channels, mid_channels, kernel_size=1, stride=1, padding=0, groups=1,
                                    QuantType=QuantType, WScale=WScale, NormType=NormType, quantscale=quantscale)
        self.pointwise_conv = BitConv2d(mid_channels, out_channels, kernel_size=1, stride=1, padding=0,
                                        QuantType=QuantType, WScale=WScale, NormType=NormType, quantscale=quantscale)
        self.scale = BitParameter(torch.Tensor(1))
        self.upsample = nn.Upsample(scale_factor=2, mode='nearest')
        self.expand_conv = BitConv2d(out_channels, in_channels, kernel_size=1, stride=1, padding=0, QuantType=QuantType,
                                     WScale=WScale, NormType=NormType, quantscale=quantscale)
...
class QAttn_BN_Block(nn.Module):
    def __init__(self, in_channels, mid_channels_0, out_channels_0, mid_channels_1, out_channels_1, QuantType='4bitsym',
                 WScale='PerTensor', NormType='RMS', quantscale=0.25, test=0):
        super(QAttn_BN_Block, self).__init__()
        self.layer1 = QAttentionCondenser(in_channels, mid_channels_0, out_channels_0, QuantType=QuantType,
                                          WScale=WScale, NormType=NormType, quantscale=quantscale)
        self.layer2 = BitBatchNorm2d(in_channels, QuantType=QuantType, WScale=WScale, NormType=NormType,
                                     quantscale=quantscale)
        self.layer3 = QAttentionCondenser(in_channels, mid_channels_1, out_channels_1, QuantType=QuantType,
                                          WScale=WScale, NormType=NormType, quantscale=quantscale)
        self.layer4 = BitBatchNorm2d(in_channels, QuantType=QuantType, WScale=WScale, NormType=NormType,
                                     quantscale=quantscale)
        self.test = test
...
class AttentionCondenser(nn.Module):
    ...
    def forward(self, x):
        residual = x
        Q = self.condense(x)
        if self.test: log_tensor(f"{self.name}_MaxPool2d", Q)
        K = F.relu(self.group_conv(Q))
        if self.test: log_tensor(f"{self.name}_Conv1", K)
        K = F.relu(self.pointwise_conv(K))
        if self.test: log_tensor(f"{self.name}_Conv2", K)
        A = self.upsample(K)
        if self.test: log_tensor(f"{self.name}_Upsample", A)
        A = self.expand_conv(A)
        if self.test: log_tensor(f"{self.name}_Conv3", A)
        S = torch.sigmoid(A)
        if self.test: log_tensor(f"{self.name}_Sigmoid", S)
        V_prime = residual * S * self.scale
        V_prime += residual
        return V_prime
