...
class Attn_Condenser_Block(nn.Module):
    def __init__(self, name, in_channels, mid_channels_0, out_channels_0,
        ...
        self.group_conv_0 = nn.Conv2d(in_channels, mid_channels_0, kernel_size=5,
        ...
        self.group_conv_1 = nn.Conv2d(out_channels_0, mid_channels_1, kernel_size=5,
    ...
    def forward(self, x):
        V_0 = x
        Q_0 = self.pool_0(V_0)
        K_0 = F.relu(self.group_conv_0(Q_0))
        K_0 = F.relu(self.pointwise_conv_0(K_0))
        A_0 = self.unpool_0(K_0)
        if self.batch_norm:
            A_0 = self.batch_nrom_0(A_0)
        V_1 = V_0 * self.S + self.expand_conv_0(A_0)
        Q_1 = self.pool_1(A_0)
        K_1 = F.relu(self.group_conv_1(Q_1))
        K_1 = F.relu(self.pointwise_conv_1(K_1))
        A_1 = self.unpool_1(K_1)
        if self.batch_norm:
            A_1 = self.batch_nrom_1(A_1)
        V_prime = V_1 * self.S + A_1
        return V_prime
